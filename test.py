...
log_with_director = attributes_filter.apply_events(log,
                                                   ["Permit REJECTED by DIRECTOR", "Declaration REJECTED by DIRECTOR",
                                                    "Declaration FINAL_APPROVED by DIRECTOR",
                                                    "Permit FINAL_APPROVED by DIRECTOR"], parameters={
        attributes_filter.Parameters.ATTRIBUTE_KEY: "concept:name", attributes_filter.Parameters.POSITIVE: True})
...
#%%
#graphs for correlation between Requested Amount and Throughput Time; Number of Approvals and Throughput Time in 2017 and 2018

data = read_csv("Data_in_2017.csv", delimiter=",")
to_be_plotted = ['Amount', 'Number Of Levels']
for feature in to_be_plotted:
    x = data[feature]
    y = data['Throughput Time']
    ax1 = sns.regplot(y=y, x=x, color='darkred')
    plt.show()
#%%
data = read_csv("Data_in_2018.csv", delimiter=",")
to_be_plotted = ['Amount', 'Number Of Levels']
for feature in to_be_plotted:
    x = data[feature]
    y = data['Throughput Time']
    
    ax2 = sns.regplot(y=y, x=x, color='darkred')
   plt.show()
#%%
#graphs for Requested Amount in 2017 and 2018

data = read_csv("Data_in_2017.csv", delimiter=",")
data.boxplot('Amount', return_type='axes')
#plt.show()
data = read_csv("Data_in_2018.csv", delimiter=",")
data.boxplot('Amount', return_type='axes')
#plt.show()

#graphs for Throughput Time in 2017 and 2018

data = read_csv("Data_in_2017.csv", delimiter=",")
data.boxplot('Throughput Time', return_type='axes')
#plt.show()
data = read_csv("Data_in_2018.csv", delimiter=",")
data.boxplot('Throughput Time', return_type='axes')
#plt.show()
#%%
#graphs for Number of Approvals in 2017 and 2018
data = read_csv("Data_in_2017.csv", delimiter=",")
data['Number Of Levels'].value_counts().plot.bar()
plt.show()
data = read_csv("Data_in_2018.csv", delimiter=",")
data['Number Of Levels'].value_counts().plot.bar()
plt.show()
#%%
#graphs for Number of Approved/Rejected in 2017 and 2018

data = read_csv("Data_in_2017.csv", delimiter=",")
data['APPROVED/REJECTED'].value_counts().plot.bar()
plt.show()
data = read_csv("Data_in_2018.csv", delimiter=",")
data['APPROVED/REJECTED'].value_counts().plot.bar()
plt.show()
#%%
#graphs for Number of Director Involved in 2017 and 2018

data = read_csv("Data_in_2017.csv", delimiter=",")
data['DIRECTOR Involved'].value_counts().plot.bar()
plt.show()
data = read_csv("Data_in_2018.csv", delimiter=",")
data['DIRECTOR Involved'].value_counts().plot.bar()
plt.show()
